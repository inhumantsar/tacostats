...
from tacostats.statsio_backends import local, s3
from tacostats.statsio_backends.s3 import get_dt_prefix
...
def read_comments(dt_date: Union[None, date] = None) -> List[Comment]:
    """Read comments file"""
    return [Comment.from_dict(c) for c in read(get_dt_prefix(dt_date), COMMENTS_KEY)]


def read_user_comments(username: str, dt_date: Union[None, date] = None) -> List[Comment]:
    """Read user comments for a particular day"""
    return [Comment.from_dict(c) for c in read(get_dt_prefix(dt_date), username)]
